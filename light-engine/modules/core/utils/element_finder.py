...
async def flatten_page_to_html(page: Page) -> str:
    """
    ...
    async def scan_frame(frame, depth=0):
        ...
        try:
            ...
            # Note: frame.evaluate is async
            main_html = await frame.evaluate("""
                () => {
        ...
        try:
            # query_selector_all is async? No, it returns ElementHandle, but in python async api?
            # page.query_selector_all returns List[ElementHandle]. Await is needed.
            ...
            for idx, iframe_element in enumerate(iframe_elements):
                try:
                    ...
                    if content_frame:
                        ...
                        iframe_html = await scan_frame(content_frame, depth + 1)
    ...
    return await scan_frame(page.main_frame, depth=0)


async def get_iframe_elements(page: Page, iframe_selector: str, element_selector: str):
    """
    ...
    try:
        ...
        els = await content_frame.query_selector_all(element_selector)
        return els
    except:
        return []


async def deep_find_elements(root, css_selector: str, timeout_ms: int = 15000):
    """
    ...
    while (time.time() - start_time) < timeout_seconds:
        ...
        for frame in page.frames:
            ...
            try:
                ...
                if await visible.count() > 0:
                    logger.debug(f"✓ Found {await visible.count()} visible elements after {poll_count} polls")
                    return await visible.all()
                
                # Try shadow selector
                shadow_visible = frame.locator(f"pierce/{css_selector}").locator("visible=true")
                if await shadow_visible.count() > 0:
                    logger.debug(f"✓ Found {await shadow_visible.count()} shadow elements after {poll_count} polls")
                    return await shadow_visible.all()
        ...
        await asyncio.sleep(min(poll_interval, max(0, timeout_seconds - elapsed)))
