...
class LiberoLoginPageHandler(StateHandler):
    ...
    async def handle(self, page: Page) -> FlowResult:
        try:
            ...
            try:
                if await page.locator("div#iubenda-cs-banner").is_visible():
                    ...
                    await self.automation.human_click(
                     ...
                     # Wait up to 5s
                    try:
                        await page.wait_for_selector("div#iubenda-cs-banner", state="visible", timeout=5000)
                        ...
                        self.logger.debug("No cookies banner detected within 5s", extra={"account_id": self.account.id})
            ...
            while time.time() - start_time < 300: # 5 minutes
                ...
                if await page.locator(password_selector).is_visible():
                ...
                # 2. Check for Captcha Solved Class (Deep search style - manually iterating frames)
                captcha_solved = False
                for frame in page.frames:
                    try:
                        if await frame.locator(solved_selector).count() > 0:
                            captcha_solved = True
                            break
                    except:
                        pass
...
class UnknownPageHandler(StateHandler):
    ...
    async def handle(self, page: Page) -> FlowResult:
        try:
            ...
            await navigate_to(page, "https://mail1.libero.it/")
