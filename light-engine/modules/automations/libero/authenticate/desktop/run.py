...
class LiberoAuthentication(HumanAction):
    ...
    async def _is_goal_reached(self, page: Page) -> bool:
        ...
        try:
            ...
            ad = page.locator('div.io-ox-dialog-wrapper')
            
            if await wizard.is_visible() or await ad.is_visible():
    ...
    async def execute(self) -> dict:
        ...
        try:
            ...
            result = await self.authenticate(page)
    ...
    async def _configure_extension(self, page: Page):
        ...
        try:
            ...
            # get_rektcaptcha_popup_url is synchronous, so we don't await it
            ...
            async def toggle_setting(selector):
            ...
            # Enable auto open
            await toggle_setting('div[data-settings="recaptcha_auto_open"]')
            
            # Enable auto solve
            await toggle_setting('div[data-settings="recaptcha_auto_solve"]')
    ...
    async def authenticate(self, page: Page):
        ...
        await self._configure_extension(page)

        # Navigate to Libero with retry on network errors
        await navigate_with_retry(page, "https://mail1.libero.it/", max_retries=3, account=self.account, logger=self.logger)
