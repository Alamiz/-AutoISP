...
class ChangePasswordStep(Step):
    ...
    async def run(self, page: Page) -> StepResult:
        try:
            ...
            await self._fill_current_password(page, current_password)
            
            # Fill new password
            await self._fill_new_password(page, new_password)
            
            # Confirm new password
            await self._fill_confirm_password(page, new_password)
            
            # Submit
            await self._submit_password_change(page)
            
            # Verify password change success by checking for confirmation message
            await page.wait_for_timeout(3000)
            confirm_elements = await deep_find_elements(page, '.hint-confirm#feedback', timeout_ms=5000)
    ...
    @retry_action()
    async def _fill_current_password(self, page: Page, password: str):
        elements = await deep_find_elements(
            page,
    ...
    @retry_action()
    async def _fill_new_password(self, page: Page, password: str):
        elements = await deep_find_elements(
            page,
    ...
    @retry_action()
    async def _fill_confirm_password(self, page: Page, password: str):
        elements = await deep_find_elements(
            page,
    ...
    @retry_action()
    async def _submit_password_change(self, page: Page):
        await self.automation.human_click(
            page,
...
class AddRecoveryEmailStep(Step):
    ...
    async def run(self, page: Page) -> StepResult:
        ...
        try:
            ...
            temp_mail_page = await page.context.new_page()
            await temp_mail_page.goto(TEMP_MAIL_URL)
            
            # Consent dialog handling
            try:
                self.logger.info("Checking for temp-mail consent dialog...", extra={"account_id": self.account.id})
                await temp_mail_page.wait_for_selector('div.fc-dialog-container', timeout=8000)
                ...
                await temp_mail_page.wait_for_timeout(2000)
            except Exception:
                self.logger.info("Consent dialog not found or already closed", extra={"account_id": self.account.id})

            await temp_mail_page.wait_for_timeout(3000)

            # Check if current email input matches username
            email_input = await temp_mail_page.query_selector('input#email')
            current_temp_email = await email_input.input_value() if email_input else ""
            ...
            if email_username in current_temp_email:
                ...
                await temp_mail_page.hover('input#email')
                await temp_mail_page.wait_for_timeout(1500)
                ...
                try:
                    username_button = await temp_mail_page.wait_for_selector(username_button_selector, timeout=5000)
                except:
                ...
                if username_button:
                    ...
                    await self.automation.human_click(temp_mail_page, selectors=[username_button_selector])
                    await temp_mail_page.wait_for_timeout(3000)
                else:
                    ...
                    await temp_mail_page.click('button[data-qa="change-button"]')
                    await temp_mail_page.wait_for_timeout(1500)
                    
                    # Step 3: Insert email username
                    await temp_mail_page.fill('form input', email_username)
                    await temp_mail_page.wait_for_timeout(1000)
                    
                    # Step 4: Click domain dropdown to open it
                    await temp_mail_page.click('select[data-qa="selected-domain"]')
                    await temp_mail_page.wait_for_timeout(1000)
                    
                    # Step 5: Select the first domain option
                    await temp_mail_page.click('section.flex-col > button:nth-of-type(1)')
                    await temp_mail_page.wait_for_timeout(1000)
                    
                    # Step 6: Click submit button
                    await temp_mail_page.click('button[data-qa="submit-button"]')
                    await temp_mail_page.wait_for_timeout(3000)
            
            # Step 7: Get the full temp email address
            temp_email_input = await temp_mail_page.query_selector('div.items-center.relative > input#email')
            if temp_email_input:
                recovery_email = await temp_email_input.input_value()
            else:
            ...
            await self.navigate_to_recovery_email_page(page)
            
            # Step 9: Check for existing recovery email
            email_field = await deep_find_elements(page, 'input#contactEmail', timeout_ms=5000)
            if not email_field:
                raise Exception("Recovery email input field not found")
            
            existing_email = (await email_field[0].input_value()).strip()
            ...
            if existing_email:
                ...
                await self._fill_password(page, current_password)
                
                # 2. Click submit
                await self._submit_recovery_email(page)
                
                # 3. Click delete link a[target="_self"]
                await page.wait_for_timeout(3000)
                delete_links = await deep_find_elements(page, 'a[target="_self"]', timeout_ms=5000)
                if not delete_links:
                    raise Exception("Delete link a[target='_self'] not found")
                
                await delete_links[0].click()
                await page.wait_for_timeout(2000)
                
                # REDO add steps with verification (uses retry decorator)
                await self._add_recovery_email_with_verification(page, recovery_email, current_password)
            ...
            code = await self._wait_for_verification_code(temp_mail_page)
            ...
            await self._fill_verification_code(page, code)
            
            # Step 13: Submit the verification code
            await self._submit_verification_code(page)
            
            # Step 14: Wait for confirmation icon
            await self._check_confirmation(page)
    ...
    @retry_action()
    async def navigate_to_recovery_email_page(self, page: Page):
        ...
        for _ in range(5):
            if await deep_find_elements(page, 'input#contactEmail', timeout_ms=3000):
    ...
    @retry_action()
    async def _fill_recovery_email(self, page: Page, email: str):
        elements = await deep_find_elements(
            page,
    ...
    @retry_action()
    async def _add_recovery_email_with_verification(self, page: Page, recovery_email: str, password: str):
        """
        ...
        # Fill recovery email
        email_elements = await deep_find_elements(page, 'input#contactEmail', timeout_ms=5000)
        if not email_elements:
            raise Exception("Recovery email input not found")
        await email_elements[0].fill(recovery_email)
        
        # Fill password
        password_elements = await deep_find_elements(page, 'input#password', timeout_ms=5000)
        ...
        code_input = await deep_find_elements(page, 'input#code-field-0', timeout_ms=10000)
        if not code_input:
    ...
    @retry_action()
    async def _submit_recovery_email(self, page: Page):
        await self.automation.human_click(
            page,
            selectors=['button[type="submit"]'],
            deep_search=True
        ...
        await page.wait_for_timeout(5000)  # Wait for code to be sent
    
    
    async def _wait_for_verification_code(self, temp_mail_page: Page, max_attempts: int = 10) -> str:
        """
        ...
        for attempt in range(max_attempts):
            ...
            # Reload the page
            await temp_mail_page.reload()
            await temp_mail_page.wait_for_timeout(10000)
            
            # Check for emails with matching subject
            email_items = await temp_mail_page.query_selector_all('ul.email-list > li')
            for item in email_items:
                try:
                    subject_el = await item.query_selector('span.message__subject')
                    if subject_el:
        ...
        for _ in range(5):
            ...
            code_element = await temp_mail_page.query_selector('span#verification-code')
            if code_element:
    ...
    async def _fill_verification_code(self, page: Page, code: str):
        ...
        for i, digit in enumerate(code[:6]):
            try:
                ...
                for attempt in range(10):  # Retry up to 10 times (10 seconds)
                    elements = await deep_find_elements(page, css_selector=input_selector)
                    if elements:
                        break
                    await page.wait_for_timeout(1000)
                
                if elements:
                    await elements[0].fill(digit)
    ...
    @retry_action()
    async def _submit_verification_code(self, page: Page):
    ...
    async def _check_confirmation(self, page: Page, timeout: int = 5000):
        ...
        try:
            ...
            await page.wait_for_selector(confirmation_path, timeout=timeout)
