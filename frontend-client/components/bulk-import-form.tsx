...
export function BulkImportForm({ mode, onCancel, onSuccess, initialFile, initialText }: BulkImportFormProps) {
    ...
    // Auto-process initial data
    useEffect(() => {
        if (initialFile) {
            setFile(initialFile)
            setActiveTab("file")
            processContent(initialFile, "file")
        } else if (initialText) {
            setPastedText(initialText)
            setActiveTab("paste")
            processContent(initialText, "paste")
        }
    }, [initialFile, initialText])

    const processContent = async (content: File | string, type: "file" | "paste") => {
        let rawContent = ""
        if (type === "file") {
            rawContent = await (content as File).text()
        } else {
            rawContent = content as string
        }
        parseAndSetItems(rawContent)
    }
    ...
    const parseAndSetItems = (rawContent: string) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // @ts-ignore
            const created_ids = (response.created_ids || []) as number[]
            // @ts-ignore
            const existing_ids = (response.existing_ids || []) as number[]

            const count = created_ids.length + existing_ids.length
            toast.success(`Successfully uploaded ${count} items`)

            onSuccess([...created_ids, ...existing_ids])
        } catch (error) {
    ...
    return (
        <div className="flex flex-col h-full bg-card min-h-[400px]">
            {step === 1 ? (
                <div className="h-full flex flex-col space-y-6">
                    <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)} className="w-full flex-1 flex flex-col">
                        ...
                        <TabsContent value="file" className="flex-1 flex flex-col space-y-4 data-[state=inactive]:hidden">
                        ...
                        <TabsContent value="paste" className="flex-1 flex flex-col h-full data-[state=inactive]:hidden">
                            ...
                            <Textarea
                                ...
                                className="w-full font-mono text-sm resize-none flex-1"
                            />
                        </TabsContent>
                    </Tabs>
                ...
                <div className="h-full flex flex-col space-y-4">
                    ...
                    <div className="border border-border rounded-lg overflow-hidden flex flex-col flex-1">
                        <div className="grid grid-cols-[auto_1fr_auto] gap-4 p-3 bg-muted/50 border-b border-border text-xs font-medium text-muted-foreground uppercase sticky top-0 z-10">
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <Button onClick={handleUpload} disabled={selectedCount === 0 || loading} className="flex-1">
                            {loading ? "Uploading..." : `Import ${selectedCount} Items`}
                        </Button>
                    </div>
                </div>
            )}
