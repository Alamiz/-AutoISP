...
class NavigateToSpamStep(Step):
    def run(self, page: Page) -> StepResult:
        try:
            ...
            from .steps import ReportSpamEmailsStep
            return StepResult(status=StepStatus.SUCCESS, payload=ReportSpamEmailsStep(self.human, self.logger))
    ...
    def run(self, page: Page) -> StepResult:
        try:
            ...
            if getattr(self.human, "reported_email_ids", []):
                from .steps import OpenReportedEmailsStep
                return StepResult(status=StepStatus.SUCCESS, payload=OpenReportedEmailsStep(self.human, self.logger))
...
class HandleUnknownPageStep(Step):
    def run(self, page: Page) -> StepResult:
        try:
            ...
            return StepResult(status=StepStatus.SUCCESS, payload=NavigateToSpamStep(self.human, self.logger))
        except Exception as e:
